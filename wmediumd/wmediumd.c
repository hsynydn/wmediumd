...
void rearm_timer(struct wmediumd *ctx)
{
	...
	list_for_each_entry(station, &ctx->stations, list) {
		for (i = 0; i < IEEE80211_NUM_ACS; i++) {
			...
			if (frame && (!set_min_expires || timespec_before(&frame->expires,
						      &min_expires))) {
	...
	if (set_min_expires) {
		...
		timerfd_settime(timer_fd, TFD_TIMER_ABSTIME, expires, NULL);
	}
	...
	for (i = 0; i < ctx->num_stas; i++)
		for (j = 0; j < ctx->num_stas; j++) {
			...
			ctx->intf[i * ctx->num_stas + j].prob_col =
				ctx->intf[i * ctx->num_stas + j].duration /	(double)duration;
...
static void threaded_process_messages_cb(struct wmediumd *ctx, struct nl_msg *msg) {
	...
	if (gnlh->cmd == HWSIM_CMD_FRAME) {
		...
		if (attrs[HWSIM_ATTR_ADDR_TRANSMITTER]) {
			...
			frame->sender = sender;
			sender->freq = freq;
			frame->tx_rates_count =	tx_rates_len / sizeof(struct hwsim_tx_rate);
